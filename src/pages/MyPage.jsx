...
import { fetchUserById } from '../api/users'; // 💡 API 도구 불러오기

const MyPage = () => {
  const navigate = useNavigate();

  // 💡 DB에서 가져온 진짜 유저 데이터를 담을 그릇(State)
  const [userData, setUserData] = useState(null);
  const [isProfileModalOpen, setIsProfileModalOpen] = useState(false);

  // 화면이 처음 켜질 때 1번 유저의 진짜 데이터를 불러옵니다.
  useEffect(() => {
  ...
  const handleLogout = () => {
    ...
    if (isConfirm) alert("성공적으로 로그아웃 되었습니다.");
  };

  // 데이터가 도착하기 전에는 로딩 화면을 보여줍니다.
  if (!userData) {
    return <div className="min-h-screen flex items-center justify-center bg-stone-100 font-bold text-stone-500">데이터를 불러오는 중입니다...</div>;
  }
  ...
  return (
    <div className="min-h-screen bg-stone-100 font-sans text-stone-900 max-w-md mx-auto relative pb-24">
      <header className="flex items-center justify-between px-5 py-6">
        ...
        <button onClick={handleNotification} className="p-2 text-stone-500 hover:bg-white/50 rounded-full transition-all duration-300"><Bell size={24} /></button>
      </header>

      <main className="px-4 space-y-4">
        {/* 1. Profile Section (진짜 데이터 렌더링!) */}
        <section onClick={() => setIsProfileModalOpen(true)} className="bg-white/75 backdrop-blur-xl rounded-3xl p-6 shadow-md border border-white/30 flex flex-col items-center text-center transition-all duration-300 hover:shadow-xl cursor-pointer active:scale-[0.98]">
          ...
          <div className="space-y-1 mb-4">
            <h2 className="text-2xl font-extrabold tracking-tight text-stone-900 flex items-center justify-center gap-1">
              {userData.nickname} <span className="text-base font-normal text-stone-400">#KR1</span>
            </h2>
            <div className="flex items-center justify-center space-x-2 text-sm text-stone-700">
              <span className={`font-extrabold ${getTierColor(userData.tier)}`}>{userData.tier || 'Unranked'}</span>
        ...
        {/* 2. Manner Score Card */}
        <section onClick={handleMannerScore} className="bg-white/70 backdrop-blur-xl rounded-3xl border border-white/30 shadow-md p-5 transition-all duration-300 hover:shadow-xl hover:bg-white/90 cursor-pointer active:scale-[0.98]">
          ...
          <div className="flex items-end justify-between">
            <div>
              <p className="text-2xl font-black text-stone-900 tracking-tight">
                {userData.rating || 0} <span className="text-sm font-medium text-stone-500">/ 5.0</span>
              </p>
              <p className="text-xs text-stone-500 mt-1">{userData.rating_count || 0}명 평가</p>
            </div>
            <div className="flex flex-col items-end gap-1.5">
              ...
              <span className="text-[10px] font-bold text-violet-500 bg-violet-50 px-2.5 py-1 rounded-full border border-violet-100">매우 좋아요</span>
            </div>
          </div>
        </section>

        {/* 3. Match History Card */}
        <section onClick={handleMatchHistory} className="bg-white/70 backdrop-blur-xl rounded-3xl border border-white/30 shadow-md p-5 transition-all duration-300 hover:shadow-xl hover:bg-white/90 cursor-pointer active:scale-[0.98]">
        ...
        <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-4 bg-stone-900/40 backdrop-blur-sm" onClick={() => setIsProfileModalOpen(false)}>
          <div onClick={(e) => e.stopPropagation()} className="w-full max-w-sm bg-white rounded-[2rem] p-6 shadow-2xl animate-in slide-in-from-bottom-8 duration-300">
            <div className="flex justify-between items-center mb-6">
              ...
              <button onClick={() => setIsProfileModalOpen(false)} className="p-2 bg-stone-100 text-stone-500 rounded-full hover:bg-stone-200"><X size={18} /></button>
            </div>
            <div className="space-y-4">
              <div className="flex justify-between items-center bg-stone-50 p-4 rounded-2xl">
                ...
                <span className="font-bold text-violet-500">{userData.game_mode || '-'}</span>
              </div>
              <div className="flex justify-between items-center bg-stone-50 p-4 rounded-2xl">
                <span className="text-sm font-medium text-stone-500">주 포지션</span>
                <span className="font-bold text-stone-800">{userData.line || '-'}</span>
              </div>
              <div className="flex justify-between items-center bg-stone-50 p-4 rounded-2xl">
                <span className="text-sm font-medium text-stone-500">플레이 스타일</span>
                <span className="font-bold text-orange-500">{userData.duo_style || '-'}</span>
              </div>
            </div>
     ...
     {/* Bottom Navigation */}
      <nav className="fixed bottom-0 left-0 right-0 w-full max-w-md mx-auto bg-white/80 backdrop-blur-xl border-t border-stone-200 z-40">
        <div className="flex justify-around items-center py-4">
          {/* 💡 여기에 onClick={() => navigate('/')} 추가! */}
          <button onClick={() => navigate('/')} className="flex flex-col items-center space-y-1 text-stone-400 hover:text-stone-600">
            <Home size={24} />
            <span className="text-[10px] font-medium">홈</span>
          </button>
          
          <button onClick={() => alert("채팅 기능은 준비 중입니다!")} className="flex flex-col items-center space-y-1 text-stone-400 hover:text-stone-600">
