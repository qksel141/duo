...
const MatchHistory = () => {
  ...
  // 💡 매칭 히스토리와 별점을 관리하는 상태
  const [history, setHistory] = useState([
    { id: 1, name: '페이커', tag: 'T1', date: '오늘 14:30', rating: 0 },
    { id: 2, name: '쵸비', tag: 'GEN', date: '어제 20:15', rating: 0 },
    { id: 3, name: '데프트', tag: 'KT', date: '3일 전', rating: 5 },
  ]);
  ...
  return (
    <div className="min-h-screen bg-stone-100 font-sans text-stone-900 max-w-md mx-auto relative pb-24">
      ...
      <main className="px-5 mt-6 space-y-4">
                ...
                <div className="flex gap-2 text-yellow-400">
                  {[1, 2, 3, 4, 5].map((star) => (
                    <button 
                      ...
                      className="focus:outline-none transition-all duration-300 hover:scale-125 active:scale-90"
                    >
                      <Star 
                        size={32} 
                        fill={star <= match.rating ? "currentColor" : "none"} 
                        strokeWidth={star <= match.rating ? 0 : 2} 
                        className={star <= match.rating ? "drop-shadow-sm" : "text-stone-300"} 
                      />
                    </button>
                  ))}
                </div>
        ...
        <button 
          onClick={handleSave}
          className="w-full mt-8 bg-violet-500 hover:bg-violet-600 text-white rounded-2xl shadow-lg transition-all duration-300 py-4 font-bold text-lg active:scale-[0.98]"
        >
          평가 저장하기
        </button>
      </main>
